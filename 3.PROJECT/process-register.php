...
// Check that form has been submitted:
	...
	// Check for a last name:
		$last_name = trim($_POST['displayname']);
	if (empty($last_name)) {
...
try {
	...
	// Hash password current 60 characters but can increase
		...
		require ('connect.php'); // Connect to the db.            #7
		// Make the query:                                               #8
		$query = "INSERT INTO cse ( username, displayname, email, password, registration_date) ";
		$query .="VALUES( ?, ?, ?, ?, NOW() )";	 // #9
		//$query .="VALUES( '$first_name', '$last_name', '$email', '$hashed_passcode', NOW() )";	 
